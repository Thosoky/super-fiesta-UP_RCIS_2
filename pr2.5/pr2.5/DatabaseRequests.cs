...
namespace pr2._5
{
    public class DatabaseRequests
    {
        ...
        {
            var querySql =
                $"INSERT INTO route(id_driver, id_car, id_itinerary, number_passangers) VALUES ('{idDriver}','{idCar}','{idItinerary}','{numberPassengers}')";
        ...
        public static void GetRoute(int idRoute)
        {
            ...
            Console.WriteLine($"Количество пассажиров: {reader[4]}");
        ...
        public static void AddCar(string nameCar, string state_number, int number_passenger, int id_type_car)
        {
            var querySql = $"INSERT INTO car (id_type_car, name, state_number, number_passenger) VALUES ({id_type_car}, '{nameCar}', '{state_number}', '{number_passenger}')";
            using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
            using var reader = cmd.ExecuteReader();
        ...
        public static void AddRightsCategoryQuery(string rights)
        {
            var querySql = $"INSERT INTO rights_category(name) VALUES ('{rights}')";
            using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
            cmd.ExecuteNonQuery();
        }
        ...
        public static void GetDriverRightsCategoryQuery(int driver)
        {
            var querySql = "SELECT dr.first_name, dr.last_name, rc.name"
                           ...
                           +
                           "INNER JOIN driver dr on driver_rights_category.id_rights_category"
                           +
